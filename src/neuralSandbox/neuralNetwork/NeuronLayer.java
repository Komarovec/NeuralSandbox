 ...
 */
package neuralSandbox.neuralNetwork;
...
public class NeuronLayer {
    ...
    public void paintNeurons(Graphics2D g2d, Playground pg, int offset, int distance) {
        for(int i = 0; i < this.getNeuronCount(); i++) {
            Circle2D nCir = new Circle2D(
                (pg.getMf().getPlaygroundPanel().getWidth()-pg.getScaledValue(distance)+pg.getScaledValue(offset))-pg.getLocation().x,
                ((pg.getScaledValue(30)*i) + pg.getScaledValue(30))-pg.getLocation().y, 
                pg.getScaledValue(5)
            );
